...
// Copie d'archive : chaque campagne envoyée depuis l'admin est aussi envoyée à cette adresse
const ARCHIVE_TO = process.env.ARCHIVE_TO || MAIL_USER || 'contact@soireematch.com';
...
try { db.exec('ALTER TABLE reservations ADD COLUMN refund_error TEXT'); } catch { /* déjà là */ }
...
L'équipe Soirée Match`,
  ...
  {
    name: 'Rappel — date limite d\'inscription (samedi)',
...
async function refundResa(resa, so, reason, o) {
  if (PAY_ON && resa.stripe_payment_intent) {
    try {
      ...
      console.error('Remboursement Stripe échoué', resa.id, msg);
      try { db.prepare('UPDATE reservations SET refund_error=? WHERE id=?').run(`${nowIso().slice(0, 16).replace('T', ' ')} — ${msg}`, resa.id); } catch {}
      mailAdminRefundFailed(so, resa, reason, msg);
      return false;
    }
  }
...
function mailAdminRefundFailed(so, r, reason, msg) {
  const to = adminMail();
  if (!to || !transporter) return;
  const qui = `${(r.prenom || '').trim()} ${(r.nom || '').trim()}`.trim() || r.email;
  transporter.sendMail({ from: MAIL_FROM, to,
    subject: `ÉCHEC remboursement Stripe — ${qui} — Soirée Match du ${so.date_texte || so.code || '?'}`,
    text: `Le remboursement automatique n'a PAS fonctionné.

Personne : ${qui}
E-mail : ${r.email}
Réservation n° ${r.id} — soirée ${so.code || ''} (${so.date_texte || ''})
Motif du remboursement : ${reason}
Erreur Stripe : ${msg}

La réservation est restée « payée » et la personne n'a PAS reçu d'e-mail de remboursement.
À faire : rembourser à la main dans le dashboard Stripe (paiement ${r.stripe_payment_intent || '—'}), puis cliquer « Retirer » dans l'admin si besoin.` }).catch(() => {});
}
// Avertissement à Marc : quelqu'un vient de demander son remboursement
function mailAdminRefundRequest(so, r, ok) {
  const to = adminMail();
  if (!to || !transporter) return;
  const qui = `${(r.prenom || '').trim()} ${(r.nom || '').trim()}`.trim() || r.email;
  transporter.sendMail({ from: MAIL_FROM, to,
    subject: `${ok ? 'Remboursement demandé et effectué' : 'ÉCHEC remboursement demandé'} — ${qui} — Soirée Match du ${so.date_texte || so.code || '?'}`,
    text: `${qui} (${r.email}) vient de demander le remboursement de sa place.

Soirée : ${so.code || ''} (${so.date_texte || ''})
Réservation n° ${r.id}

${ok
      ? "Le remboursement a été envoyé à Stripe automatiquement : la personne a reçu l'e-mail de confirmation, l'argent revient sur son moyen de paiement d'ici quelques jours. Rien à faire de ton côté."
      : "⚠ Le remboursement automatique a ÉCHOUÉ. La personne a été prévenue que tu traites sa demande à la main. Va la rembourser dans le dashboard Stripe."}` }).catch(() => {});
}
...
const admTok = (sid) => crypto.createHmac('sha256', SECRET).update('adm:' + sid).digest('base64url');
const resaTok = (rid) => crypto.createHmac('sha256', SECRET).update('resa-act:' + rid).digest('base64url');
const adminMail = () => NOTIFY_TO || (/@/.test(ADMIN_USER) ? ADMIN_USER : '');
...
// Soirées de remplacement proposées dans les e-mails d'annulation.
// opts = { codes: [..] | null (toutes), force: bool }. force = proposer même hors profil
// (réservé à l'e-mail des non-payés : la page /report, elle, n'accepte que les soirées du profil).
function futureSoireesOpen(excludeId) {
  const now = nowMs();
  return db.prepare('SELECT * FROM soirees WHERE actif=1 AND COALESCE(annulee,0)=0').all()
    .filter((f) => f.id !== excludeId && (!f.date_start || new Date(f.date_start).getTime() > now));
}
function proposedSoirees(person, so, opts, allowForce) {
  const codes = opts && Array.isArray(opts.codes) ? opts.codes : null;
  const force = allowForce && !!(opts && opts.force);
  let list = [];
  try { list = force ? futureSoireesOpen(so.id) : eligibleFutureSoirees(person, so.id); } catch { list = []; }
  return codes ? list.filter((f) => codes.includes(f.code)) : list;
}
// Contenu de l'e-mail « reporter ou être remboursé(e) » — partagé par l'envoi et la page d'aperçu
function cancelChoiceMail(so, r, motif, opts) {
  const prenom = (r.prenom || '').trim() || 'à toi';
...
function mailCancelChoice(so, r, motif, opts) {
  if (!transporter) return;
  const m = cancelChoiceMail(so, r, motif, opts);
...
// Journalise un envoi automatique dans la page « E-mails envoyés ».
// Le contenu étant personnalisé (dates, boutons), on enregistre le rendu d'un destinataire à titre d'exemple.
function logMails(tplName, subject, sampleText, recips, so) {
  try {
    if (!recips.length) return;
    const body = `[Envoi automatique — contenu personnalisé pour chaque destinataire. Exemple du rendu reçu par ${(recips[0].prenom || '').trim() || recips[0].email} :]\n\n${sampleText}`;
    db.prepare('INSERT INTO campaigns(sent_at,subject,body,tpl,soirees,tranches,recipients,sent,failed,recipient_list,genres) VALUES(?,?,?,?,?,?,?,?,0,?,?)')
      .run(nowIso(), subject, body, tplName, (so && so.code) || '', (so && so.tranche) || '', recips.length, recips.length,
...
async function cancelWithChoice(so, motif, opts) {
  ...
  if (payes.length) {
    const ex = cancelChoiceMail(so, payes[0], m, opts);
    logMails('Annulation — personnes ayant payé', ex.subject, ex.text, payes, so);
  }
  const autres = db.prepare("SELECT * FROM reservations WHERE soiree_id=? AND status IN ('waiting','hold')").all(so.id);
  for (const r of autres) {
    db.prepare("UPDATE reservations SET status='cancelled' WHERE id=?").run(r.id);
    mailCancel(so, r, m, opts);
  }
  if (autres.length) {
    const ex = cancelSimpleMail(so, autres[0], m, opts);
    logMails('Annulation — sans paiement', ex.subject, ex.text, autres, so);
  }
...
function decisionSaturdayMs(so) {
...
function eligibleFutureSoirees(person, excludeId) {
  const now = nowMs();
  return db.prepare('SELECT * FROM soirees WHERE actif=1 AND COALESCE(annulee,0)=0').all()
    .filter((so) => so.id !== excludeId && (!so.date_start || new Date(so.date_start).getTime() > now) && eligibleForSoiree(person, so));
}
async function tick() {
  const list = db.prepare('SELECT * FROM soirees WHERE COALESCE(annulee,0)=0').all();
  const now = nowMs();
  for (const so of list) {
    try {
      if (so.actif && expireHolds(so)) promote(so);
...
function mailSlotOpen(so, r) {
  ...
  const txt = `Bonjour ${prenom},\n\nBonne nouvelle : une place vient de se libérer pour toi pour la Soirée Match${so.date_texte ? ` du ${so.date_texte}` : ''} !\n\nTa place a été réservée en priorité, dans ton ordre d'inscription, mais nous ne pouvons pas la garder plus de trois heures pour ne pas bloquer les autres personnes de la liste.\n\nConfirme et règle ta place ici : ${link}\n\nÀ très vite\nTa team Soirée Match`;
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;max-width:520px;margin:auto;color:#1e2f30;font-size:15px;line-height:1.55"><p>Bonjour ${esc(prenom)},</p><p>Bonne nouvelle : une place vient de se libérer pour toi pour la <b>Soirée Match${so.date_texte ? ` du ${esc(so.date_texte)}` : ''}</b> !</p><p>Ta place a été réservée en priorité, dans ton ordre d'inscription, mais nous ne pouvons pas la garder plus de trois heures pour ne pas bloquer les autres personnes de la liste.</p><p><a href="${link}" style="display:inline-block;background:#2f7d8a;color:#fff;text-decoration:none;padding:12px 24px;border-radius:30px;font-weight:600">Confirmer et régler ma place</a></p><p style="font-size:13px;color:#8a9a99">Ce lien expire dans 3 heures.</p><p>À très vite<br>Ta team Soirée Match</p></div>`;
...
function mailRefund(so, r, reason) {
  ...
  const why = reason === 'annulation'
    ...
    : (reason === 'retrait'
      ? `ta place pour la Soirée Match${so.date_texte ? ` du ${so.date_texte}` : ''} n'a pas pu être maintenue`
  ...
  const sujet = `Remboursement — Soirée Match${so.date_texte ? ` du ${so.date_texte}` : ''}`;
  ...
  transporter.sendMail({ from: MAIL_FROM, to: r.email, subject: sujet, text: txt }).catch(() => {});
  logMails('Remboursement — ' + (reason === 'annulation' ? 'soirée annulée' : (reason === 'retrait' ? 'retrait par l\'organisateur' : 'parité')), sujet, txt, [r], so);
}
// Contenu de l'e-mail d'annulation aux personnes qui n'ont rien payé — partagé par l'envoi et l'aperçu
function cancelSimpleMail(so, r, motif, opts) {
  const prenom = (r.prenom || '').trim() || 'à toi';
  const dateTxt = so.date_texte || so.code;
  const m = (motif || '').trim() || cancelMotif(so);
  const person = { prenom: r.prenom, nom: r.nom, email: r.email, genre: r.genre, recherche: r.recherche, annee: r.annee };
  const futs = proposedSoirees(person, so, opts, true);
...
function mailCancel(so, r, motif, opts) {
  if (!transporter) return;
  const m = cancelSimpleMail(so, r, motif, opts);
  transporter.sendMail({ from: MAIL_FROM, to: r.email, subject: m.subject, text: m.text, html: emailShell(m.inner, unsubLink(r.email)) }).catch(() => {});
}
...
function cancelPreviewPage(so, motif, opts) {
...
function decisionCancelPage(so, t, motif, opts) {
...
function reportPage(r, t, futs) {
  const cards = futs.map((so) => `<div class=soi><div class=info><b>${esc(so.date_texte || so.code)}</b>${so.lieu ? `<div class=muted>${esc(so.lieu)}</div>` : ''}</div><form method=post action="/report?rid=${r.id}&t=${encodeURIComponent(t)}&to=${so.id}"><button class=btn>Reporter ici</button></form></div>`).join('');
  return `${siteHead('Reporter mon inscription')}<div class=box><h1>Reporter ta place 💛</h1><p>Choisis la prochaine soirée qui te convient — ta place y sera <b>garantie</b>, sans repayer :</p>${cards}<p style="margin-top:14px"><a href="${SITE_URL}/rembourser?rid=${r.id}&t=${encodeURIComponent(t)}">Je préfère être remboursé(e)</a></p></div></html>`;
}
function rembourserPage(r, t) {
  const q = `rid=${r.id}&t=${encodeURIComponent(t)}`;
  return `${siteHead('Remboursement')}<div class=box><h1>Confirmer le remboursement</h1><p>Tu es sur le point de demander le remboursement de ta place${r.amount ? ` (${(r.amount/100).toFixed(0)} CHF)` : ''}. Le montant réapparaîtra sur ton moyen de paiement d'ici quelques jours.</p><form method=post action="/rembourser?${q}" style="margin-top:16px"><button class=btn style="background:#c0392b">Oui, me rembourser</button></form><p style="margin-top:12px"><a href="${SITE_URL}/report?rid=${r.id}&t=${encodeURIComponent(t)}">Finalement, je préfère reporter ma place</a></p></div></html>`;
}
const server = http.createServer(async (req, res) => {
  ...
  // Décision admin (lien signé dans l'e-mail du samedi)
  if (p === '/decision' || p.startsWith('/decision/')) {
    const sid = Number(url.searchParams.get('sid') || 0);
    const t = String(url.searchParams.get('t') || '');
    const good = sid ? admTok(sid) : '';
    const valid = !!good && t.length === good.length && crypto.timingSafeEqual(Buffer.from(t), Buffer.from(good));
    const so = valid ? getSoireeById(sid) : null;
    if (!so) return send(res, 200, pubMsg('Lien invalide', 'Ce lien de décision n\'est plus valide.'));
    ...
    if (p === '/decision/cancel' && req.method === 'GET') {
      ...
      return send(res, 200, decisionCancelPage(so, t, url.searchParams.get('motif') || '', optsFromQuery(url.searchParams)));
    }
    if (p === '/decision/cancel' && req.method === 'POST') { const d = parseForm(await readBody(req)); if (!so.annulee) await cancelWithChoice(so, d.motif || '', optsFromForm(d)); return send(res, 200, pubMsg('Soirée annulée', 'Chaque personne payée a reçu le choix : reporter ou être remboursée. 💛')); }
    return send(res, 404, 'Introuvable');
  }

  // Reporter une inscription (après annulation) — lien signé
  if (p === '/report') {
    const rid = Number(url.searchParams.get('rid') || 0);
    const t = String(url.searchParams.get('t') || '');
    const good = rid ? resaTok(rid) : '';
    const valid = !!good && t.length === good.length && crypto.timingSafeEqual(Buffer.from(t), Buffer.from(good));
    const r = valid ? db.prepare('SELECT * FROM reservations WHERE id=?').get(rid) : null;
    if (!r) return send(res, 200, pubMsg('Lien invalide', 'Ce lien n\'est plus valide.'));
    if (r.status === 'refunded') return send(res, 200, pubMsg('Déjà remboursé', 'Cette place a déjà été remboursée.'));
    const person = { prenom: r.prenom, nom: r.nom, email: r.email, genre: r.genre, recherche: r.recherche, annee: r.annee };
    if (req.method === 'POST') {
      await readBody(req);
      const to = Number(url.searchParams.get('to') || 0);
      const target = to ? getSoireeById(to) : null;
      if (!target || !eligibleFutureSoirees(person, r.soiree_id).some((x) => x.id === to)) return send(res, 200, pubMsg('Oups', 'Cette soirée n\'est plus disponible. Réessaie depuis ton e-mail.'));
      db.prepare("UPDATE reservations SET soiree_id=?, status='paid', paid=1, forced=1, paid_at=COALESCE(paid_at, ?) WHERE id=?").run(to, nowIso(), r.id);
      mailReportConfirme(target, r);
      return send(res, 200, pubMsg('C\'est reporté ! 🎉', `Ta place est garantie pour la soirée du ${esc(target.date_texte || target.code)} — rien à repayer. À très vite ! 💛`));
    }
    const futs = eligibleFutureSoirees(person, r.soiree_id);
    if (!futs.length) return send(res, 200, pubMsg('Pas encore de date', `Aucune prochaine soirée de ton profil n\'est ouverte pour l\'instant. Tu peux te faire rembourser ici : ${SITE_URL}/rembourser?rid=${r.id}&t=${encodeURIComponent(t)} — ou on te préviendra dès qu\'une date s\'ouvre.`));
    return send(res, 200, reportPage(r, t, futs));
  }

  // Remboursement (après annulation) — lien signé + page de confirmation
  if (p === '/rembourser') {
    const rid = Number(url.searchParams.get('rid') || 0);
    const t = String(url.searchParams.get('t') || '');
    const good = rid ? resaTok(rid) : '';
    ...
    const r = valid ? db.prepare('SELECT * FROM reservations WHERE id=?').get(rid) : null;
    if (!r) return send(res, 200, pubMsg('Lien invalide', 'Ce lien n\'est plus valide.'));
    if (r.status === 'refunded') return send(res, 200, pubMsg('Déjà remboursé', 'Ton remboursement a déjà été lancé. 💛'));
    if (req.method === 'POST') {
      await readBody(req);
      const so = getSoireeById(r.soiree_id);
      const ok = await refundResa(r, so || {}, 'annulation');
      mailAdminRefundRequest(so || {}, r, ok);
      return ok
        ? send(res, 200, pubMsg('Remboursement lancé ✓', 'Ton remboursement est en cours — il réapparaîtra sur ton moyen de paiement d\'ici quelques jours. Merci de ta compréhension. 💛'))
        : send(res, 200, pubMsg('Demande bien reçue', 'Ta demande de remboursement est enregistrée. Un souci technique nous empêche de la traiter automatiquement : on s\'en occupe à la main et tu recevras une confirmation dès que c\'est fait. Merci de ta patience. 💛'));
    }
    return send(res, 200, rembourserPage(r, t));
  }
  ...
  if (p.startsWith('/admin')) {
    ...
    if (p === '/admin/soirees/cancel' && req.method === 'GET') {
      ...
      return send(res, 200, cancelPreviewPage(s, url.searchParams.get('motif') || '', optsFromQuery(url.searchParams)));
    }
    if (p === '/admin/soirees/cancel' && req.method === 'POST') {
      const d = parseForm(await readBody(req));
      const id = Number(d.id);
      const s = id && getSoireeById(id);
      if (s && !s.annulee) await cancelWithChoice(s, d.motif || '', optsFromForm(d));
      return send(res, 302, '', { Location: id ? `/admin/soirees/reservations?id=${id}&done=1` : '/admin/soirees' });
    }
    if (p === '/admin/soirees/confirm' && req.method === 'POST') {
      const id = Number(parseForm(await readBody(req)).id);
      const s = id && getSoireeById(id);
      if (s && !s.annulee) confirmSoiree(s);
      return send(res, 302, '', { Location: id ? `/admin/soirees/reservations?id=${id}&done=1` : '/admin/soirees' });
    }
    if (p === '/admin/soirees/close' && req.method === 'POST') {
      const id = Number(parseForm(await readBody(req)).id);
      if (id) db.prepare('UPDATE soirees SET actif=0 WHERE id=?').run(id);
      return send(res, 302, '', { Location: `/admin/soirees/reservations?id=${id}&done=1` });
    }
    if (p === '/admin/soirees/reopen' && req.method === 'POST') {
      const id = Number(parseForm(await readBody(req)).id);
      if (id) db.prepare('UPDATE soirees SET actif=1 WHERE id=?').run(id);
      return send(res, 302, '', { Location: `/admin/soirees/reservations?id=${id}&done=1` });
    }
    if (p === '/admin/soirees/resa/remove' && req.method === 'POST') {
      ...
      const so0 = getSoireeById(r.soiree_id);
      let libere = true;
      if (r.status === 'paid') {
        // passe par refundResa : remboursement Stripe, e-mail au client, journal, alerte si échec
        libere = await refundResa(r, so0 || {}, 'retrait');
      } else {
        db.prepare("UPDATE reservations SET status='cancelled', paid=0 WHERE id=?").run(id);
      }
      const so = so0;
      // si le remboursement a échoué, la place reste occupée : ne pas promouvoir quelqu'un à sa place
      if (so && libere) promote(so);
